...
public class Shapes {
	...
	public static float[] getBoxVertices(float x, float tx, int snapx, float y, float z, float tz, int snapz, boolean normals, boolean texcoords, float texScale)
	{
		int vertSize = 3;
		...
		for (int i = 0; i < vertices.length;) {
			...
			if (texcoords)
			{
				vertices[i++] = cubeTex[tIdx++] * texScale;
				vertices[i++] = cubeTex[tIdx++] * texScale;
			}
		}
	...
	public static Mesh getBoxMesh(float x, float tx, int snapx, float y, float z, float tz, int snapz, boolean normals, boolean texcoords, float texScale)
	{
		float[] vertices = getBoxVertices(x, tx, snapx, y, z, tz, snapz, normals, texcoords, texScale);
	...
	{
		return getBoxMesh(x, x, 0, y, z, z, 0, normals, texcoords, 1);
	}
